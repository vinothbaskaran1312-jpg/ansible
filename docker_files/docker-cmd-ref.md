...
docker run -it [IMAGE] (shellpath) â€“ starts a container, allocates a pseudo-TTY connected to the containerâ€™s stdin, and creates an interactive bash shell in the container.

docker run -it-rm [IMAGE] (shellpath) â€“ creates, starts, and runs a command inside the container. Once it executes the command, the container is removed.
...
docker build -t â€“ builds an image from a Dockerfile in the current directory and tags the image
